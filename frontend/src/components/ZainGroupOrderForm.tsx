// frontend/src/components/ZainGroupOrderForm.tsx
...
const initialState = {
  ...
  notes: '',
  design_cost_usd: 0,
  separation_cost_usd: FIXED_SEPARATION_COST_USD_ZAIN,
  total_cost_usd: FIXED_SEPARATION_COST_USD_ZAIN,
  is_approved: false,
};

function ZainGroupOrderForm({ clientId, onOrderCreated }: Props) {
  const [formData, setFormData] = useState(initialState);
  const [availableCodes, setAvailableCodes] = useState<Code[]>([]);

  const fetchAvailableCodes = () => {
    api.get(`/codes?client_type=Zain Group&status=available`)
      .then(setAvailableCodes)
      .catch(err => toast.error("Could not fetch available codes for Zain Group."));
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const ZAIN_EXCHANGE_RATE = 1500;
    const total_cost_iqd = (formData.total_cost_usd || 0) * ZAIN_EXCHANGE_RATE;
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <footer className="modal-footer">
        ...
        <button type="submit" className="button-primary">Add Zain Group Order</button>
      </footer>
    </form>
  );
}

export default ZainGroupOrderForm;